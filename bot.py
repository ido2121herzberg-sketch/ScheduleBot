...
**חוקי ברזל (גוברים על כל אופטימיזציה):**
...
def validate_schedule(schedule, fixed_by_day, events, recurring=None):
    ...
    if recurring:
        ...
        for t in recurring:
            ...
            if not nm or freq == "פעם בשבועיים":
                continue
            words = [w for w in nm.split() if len(w) >= 3]
            if not words:
                continue
...
def merge_open_windows(schedule):
    """Merge adjacent same-energy open windows and drop windows shorter than 30 min."""
    for day in schedule:
        ...
        for b in merged:
            st, en = _to_min(b.get("start", "")), _to_min(b.get("end", ""))
            if is_open(b) and st is not None and en is not None and (en - st) < 30:
                continue
            cleaned.append(b)
        day["blocks"] = cleaned
    return schedule
