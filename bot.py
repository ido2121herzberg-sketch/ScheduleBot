...
**אימון כושר:**
- 3 פעמים בשבוע, כל בלוק בדיוק 2.5 שעות (150 דקות).
- רק בתחילת השבוע: ראשון, שני, שלישי. לעולם לא ברביעי/חמישי/שישי/שבת.
- לעולם לא ביום שיש בו וולט, לעולם לא ביום הסשן עם תמיר, ולעולם לא חופף 'מענה לתלמידים'.
- ראשון 09:00-11:30 כולל מקלחת. בשני/שלישי 12:00-14:00.
- אם אין שלושה ימים מתאימים בתחילת השבוע בלי וולט ובלי חפיפה — שבץ פחות פעמים והשאר חוסר. אל תכריח אימון על יום אסור.
...
**כתיבת שירים:**
- אורך כל בלוק הוא תמיד בדיוק 180 דקות (3 שעות רצופות) בערב. אל תקצר ל-90 דקות בשום מקרה. לדוגמה 20:00-23:00.
- בדיוק פעמיים בשבוע, בשני ימים שונים. בלוק יחיד ורצוף בכל פעם.
- אסור לפצל. אסור 'חלק א/ב'. אסור פעמיים באותו יום.
...
3. שבץ את המשימות החוזרות הגמישות בחלונות הפנויים לפי האנרגיה והזמן המועדף.
4. מהאינבוקס שבץ אך ורק משימות עם יום מפורש. אל תדחוף משימות אינבוקס כלליות לזמן פנוי.
5. כל זמן פנוי שנשאר — סמן כבלוק "חלון פתוח" עם "type": "חלון פתוח" וסיווג אנרגיה (🟢 בוקר/אחרי מנוחה, 🟡 אמצע יום, 🔴 ערב מאוחר/אחרי עומס).
...
def validate_schedule(schedule, fixed_by_day, events):
    ...
    # 4. Songwriting: exactly 180 continuous min, max twice/week, never twice in a day.
    song_days = {}
    song_total = 0
    for day in schedule:
        for b in day.get("blocks", []):
