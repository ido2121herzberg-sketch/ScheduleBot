...
- time: HH:MM או "גמיש". אם עידו נותן שעה מדויקת עם דקות (למשל 15:15, 09:05) — שמור אותה בדיוק כפי שהיא. אם השעה מעורפלת/עגולה — עגל ל-:00/:30. לעולם לא 23:59.
- אם עידו מזכיר פעילות עם יום מפורש שהוא רוצה לעשות ("שיעור ריקוד בשישי", "הגעה למשרדים בשלישי") → זה אירוע ב-events באותו יום. ולעולם אל תשמיט אותו. (אם יש שלילה — "בלי"/"אין"/"לא" — זה skip, לא אירוע.) אם יש "במקום יום_ישן" — הוסף גם skip ביום_ישן (ראה כלל swap בסעיף directives).
- טווח ימים או העדפה בלי יום בודד מפורש ("בין ראשון לשישי", "מתישהו השבוע", "עדיף תחילת שבוע") → days=[] (אירוע גמיש; המנוע יבחר יום). אל תנעל ליום אחד.
- שעה מעורפלת → שעה עגולה: "בוקר"→"09:00", "בוקר צהריים"/"לפנהצ"→"12:00", "צהריים"→"12:00", "אחהצ"/"אחר הצהריים"→"16:00", "ערב"→"19:00".
- "כמו ביום X" / "כמו תמיד" → אותה שעה שהפעילות מקבלת בדרך כלל.
...
- swap (events + directives ביחד): כשהמשתמש אומר "X ב-יום_חדש במקום יום_ישן" / "מעביר X ל-יום_חדש" / "לא ב-יום_ישן אלא ב-יום_חדש" — חובה לייצר שני דברים:
  (1) ב-events: אירוע עבור X ב-יום_חדש כרגיל.
  (2) ב-directives: {{"action":"skip","target":"<השם המדויק מהרשימה שמתאים ל-X>","day":"<יום_ישן>"}}.
  ללא הskip, X ייתקבע בשני הימים — זה שגוי. דוגמה: "נסיעה למשרד בשלישי במקום ראשון" → events:[{{"name":"נסיעה למשרד","days":["שלישי"],"time":"גמיש","recurring":false,"energy":"","priority":"","duration_min":null}}] + directives:[{{"action":"skip","target":"<שם_בלוק_המשרד_מהרשימה>","day":"ראשון"}}].
...
def build_week(fixed, recurring, events, dated_inbox=None, rest_day=DEFAULT_REST_DAY, anchor=None, off_days=None, day_skips=None):
    ...
    for day, plan in plans.items():
        real = [x for x in plan.sorted_blocks() if not x.get("parallel") and not x.get("span")]
        if not real:
