...
**מענה ראשון לתלמידים:**
- אורך הבלוק הוא תמיד בדיוק 90 דקות (שעה וחצי). אל תקצר לשעה בשום מקרה.
- ראשון 11:30-13:00. שני עד חמישי 11:00-12:30.
- ביום הסשן עם תמיר: מתבצע תוך כדי הנסיעה החל מ-11:00, גם כן 90 דקות, ומסומן "parallel": true.
...
**אימון כושר (חדר כושר):**
- 3 פעמים בשבוע, כל בלוק רגיל בדיוק 2.5 שעות (150 דקות).
- ראשון 09:00-11:30 כולל מקלחת. בשאר ימי תחילת השבוע 13:00-15:30.
- מקם בתחילת השבוע (ראשון–שלישי) כברירת מחדל.
- ביום הסשן עם תמיר: חדר הכושר בתל אביב בערב (20:00-22:00, בלוק נפרד) נחשב כאחד משלושת ימי הכושר. אל תשבץ אימון כושר נוסף בבוקר/צהריים באותו יום.
- לעולם לא ביום שיש בו וולט, ולעולם לא חופף 'מענה לתלמידים'.

**סשן עם תמיר מפיק:**
- פעם בשבוע, יום שני עד חמישי, 14:00-18:30. בחר את היום הכי פנוי והכי קל.
- יציאה מהבית ב-11:00. הנסיעה למודיעין היא בלוק יחיד בשם "נסיעה למודיעין" עם משך עגול — אל תפצל אותה ל'הליכה' + 'נסיעה', אל תמציא משך לכל קטע ואל תמציא אמצעי תחבורה.
- 'מענה ראשון תוך כדי הנסיעה' = מתבצע במקביל לנסיעה. סמן אותו "parallel": true על אותה משבצת זמן, 90 דקות.
- מענה שני בדרך חזרה 18:30-20:00; חדר כושר בתל אביב 20:00-22:00.
- ביום הזה — יום קל בלבד, בלי משימות כבדות לפני ואחרי, ובלי אימון כושר בוקר/צהריים.

**שיעור מסחר עם שחף:**
- שעה אחת, חמישי 18:00-19:00 בעדיפות. אם יש התנגשות — יום אחר באותו שבוע.

**כתיבת שירים:**
- אורך הבלוק הוא תמיד בדיוק 180 דקות (3 שעות רצופות) בערב. אל תקצר ל-90 דקות בשום מקרה. לדוגמה 20:00-23:00.
- פעם אחת בשבוע. בלוק יחיד ורצוף.
- אסור לפצל. אסור 'חלק א/ב'.
...
**חלונות פתוחים (אנרגיה):**
- אל תשבץ משימות אינבוקס כלליות לתוך זמן פנוי. מהאינבוקס שבץ אך ורק משימות עם יום מפורש.
- כל זמן פנוי שנשאר (אחרי בלוקים קבועים + משימות חוזרות + עוגנים) — סמן כבלוק בשם "חלון פתוח" עם "type": "חלון פתוח" וסיווג אנרגיה לפי שעת היום והעומס מסביב:
  בוקר / אחרי מנוחה = 🟢, אמצע יום = 🟡, ערב מאוחר / אחרי עומס = 🔴.
- המשתמש ימלא את החלונות האלה בזמן אמת לפי האנרגיה שלו. אל תנחש עבורו ואל תכניס לשם משימות מהאינבוקס.

**עקרונות חכמים (היגיון, לא רק חוקים נוקשים):**
- **מיקום:** לכל משימה יש מקום. נגינה, אימון על שירים, כתיבת מוזיקה — דורשים בית או סטודיו, אי אפשר לעשות אותם במשרד. כשהמשתמש בבלוק עבודה/משרד — שבץ שם רק משימות עבודה או משימות מחשב, לעולם לא מוזיקה. אל תשבץ משימה במקום שלא מתאים לה.
- **נסיעה בתחבורה ציבורית:** משימות שאפשר לעשות תוך כדי נסיעה (מענה לתלמידים, קריאה, שיחות טלפון) יכולות לרוץ במקביל לנסיעה בתחבורה ציבורית — סמן "parallel": true. זו לא חפיפה אסורה ולא שגיאה.
- **זמני מעבר:** בין שני בלוקים במקומות שונים השאר זמן נסיעה. אחרי חדר כושר או יציאה מהבית יש דרך — אל תניח שאפשר להתחיל מיד את המשימה הבאה. בזמן המעבר אפשר לשבץ משימה שאפשר לעשות בדרך.
- **התאמה לשעת היום:** אל תשבץ עבודה יצירתית תובענית (יצירת תוכן, כתיבת שירים, נגינה) אחרי 22:00 — עדיף בוקר או צהריים. שום דבר לא מתוזמן אחרי 23:30, זו שעת השינה.
- **משימות מיקרו:** משימה כמו "לשלוח הודעה" לוקחת 2-5 דקות — לעולם אל תיתן לה בלוק של חצי שעה. סמן "parallel": true והצמד לבלוק סמוך. אל תקצה לה זמן בלעדי.
- **בלי פיצול שגרות:** כל מופע של שגרה הוא בלוק רצוף אחד באורך המלא שלה, לא כמה חתיכות קצרות.


- שני בלוקים אמיתיים (לא מיקרו) לעולם לא חופפים. אסור לגעת בבלוקים הקבועים או לשנות את שעותיהם. אל תפצל משימות שצריכות להיות רצופות.
- אירוע שהמשתמש נתן לו יום מפורש (חתונה, מסיבה, פגישה) = עוגן קבוע. שבץ אותו בדיוק באותו יום. לעולם אל תזיז אותו ליום אחר ולעולם אל תציע יום אחר. אם יש התנגשות באותו יום — סמן את ההתנגשות, אל תעביר את העוגן ליום פנוי.
- אירוע אחד = בלוק אחד רצוף. לעולם אל תפצל אירוע לשני בלוקים צמודים. '(המשך)' מותר אך ורק כשבלוק קבוע אחר מפצל אותו פיזית באמצע — לעולם לא כריפוד.
- כל השעות מעוגלות ל-:00 או :30. לעולם לא 23:59 ולעולם לא ריפוד עד סוף היום. אם אין שעת סיום ידועה — השתמש במשך ברירת מחדל עגול לפי סוג האירוע (אירוע ערב כמו חתונה = 4 שעות), עד שעה עגולה.
- נסיעה היא תמיד בלוק יחיד עם משך עגול. אל תפצל לקטעים ('הליכה' + 'נסיעה') ואל תמציא אמצעי תחבורה או משכי-ביניים."""
...
---
בנה לוח שבועי מלא:
1. העתק את כל הבלוקים הקבועים בדיוק.
2. שבץ את החריגים/עוגנים — כל אחד ביום שניתן לו בדיוק. עוגן עם יום מפורש לא זז ליום אחר לעולם.
3. שבץ כל אחת מהמשימות החוזרות מספר הפעמים שמתחייב מהתדירות: 'יומי' = בכל יום מהימים המועדפים; 'שבועי' = פעם אחת; 'x2 בשבוע' = פעמיים בשני ימים שונים; 'x3 בשבוע' = שלוש פעמים בשלושה ימים שונים; 'פעם בשבועיים' = רק אם המשתמש ציין בחריגים שזה פעיל השבוע. אלה התחייבויות — לא אופציה ולא "אם יש זמן". חובה שכולן יופיעו. השתמש במשך ובחלון שכתובים ב'זמן מועדף' (לדוגמה "13:00-16:30 / משך: שעה אחת" = בלוק של שעה אי-שם בין 13:00 ל-16:30). אם אין משך כתוב — בלוק של שעה.
4. מהאינבוקס שבץ אך ורק משימות עם יום מפורש. אל תדחוף משימות אינבוקס כלליות לזמן פנוי — הן יישארו לבחירה בזמן אמת.
5. רק זמן פנוי אמיתי שנשאר אחרי כל מה שלמעלה (רצף של 30 דקות ומעלה) — סמן כבלוק יחיד "חלון פתוח" עם "type": "חלון פתוח" וסיווג אנרגיה (🟢 בוקר/אחרי מנוחה, 🟡 אמצע יום, 🔴 ערב מאוחר/אחרי עומס). אל תיצור שני חלונות פתוחים צמודים — אחד אותם לבלוק אחד. אל תיצור חלון קצר מ-30 דקות.
בלי שום חפיפה בין משימות אמיתיות. משימות מיקרו (parallel) מותר שיחפפו.
שים לב במיוחד: 'מענה ראשון' חייב להיות 90 דקות, 'כתיבת שירים' חייב להיות 180 דקות רצופות, אירוע אחד = בלוק רצוף אחד, וכל השעות מעוגלות ל-:00/:30 (אף פעם לא 23:59).
...
{{
  "schedule": [
    {{"day": "ראשון", "blocks": [
        ...
        {{"start": "16:00", "end": "18:00", "name": "חלון פתוח", "energy": "🟡", "type": "חלון פתוח", "parallel": false}}
    ]}}
  ],
...
סמן "parallel": true רק למשימות מיקרו קצרות (כמה דקות) ולמענה ראשון ביום תמיר, אחרת false."""
